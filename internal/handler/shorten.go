...
func (s server) shortenText(w http.ResponseWriter, r *http.Request) {
	...
	// 2️⃣service
	slug := random.NewRandomString(config.AliasLength)
	// Запись в db
	s.repo.Create(string(body), slug)
	// s.repo[slug] = string(body)
	...
	w.Write([]byte(s.cfg.BaseURL + "/" + slug))
