...
// Неименованные поля (embedding (встраивание) usecase.Repository в Repository) используется для того, чтобы:
...
// В данном слое (инфраструктурном репозитории) Repository не возвращается в обычном понимании, а предоставляет доступ к данным.
// Используя неименованные поля мы автоматически делегируем вызовы нижележащему кэшу или базе данных.
